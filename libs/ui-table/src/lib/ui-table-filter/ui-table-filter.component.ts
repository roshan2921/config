...
export class UiTableFilterComponent {
  ...
  private updateUsedFilterItems(): void {
    const keys = [
      ...new Set(
        Object.keys(this.originalCriteria).concat(Object.keys(this.criteria))
      ),
    ];
    this.usedFilterItems = keys.filter(
      (key) =>
        JSON.stringify(this.originalCriteria[key]) !==
        JSON.stringify(this.criteria[key])
    ).length;
  }
}
