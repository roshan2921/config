...
export class UiTableComponent implements OnInit {
  ...
  SetCriteria() {
    debugger;
    this.originalCriteriaValue = this.deepCloneService.clone(
      this.tableConfig?.criteria
    );
  ...
  Search() {
    ...
    if (this.tableConfig)
      this.subscription = this.tableConfig
  ...
  resetFilter() {
    ...
    Object.keys(this.criteria).forEach((key) => {
      this.criteria[key] = this.deepCloneService.clone(
        this.originalCriteriaValue[key]
      );
    });
